...
class YoloDetector(context: Context, modelName: String) : AutoCloseable {
    ...
    fun analyze(imageProxy: ImageProxy): List<DetectedObject> {
        ...
        val buf =
            ByteBuffer.allocateDirect(640 * 640 * 3 * 4).apply { order(ByteOrder.nativeOrder()) }
    ...
    private fun applyNMS(
        ...
        while (sorted.isNotEmpty()) {
            ...
            sorted.removeAll { other ->
                val iL = maxOf(best.left, other.left);
                val iT = maxOf(best.top, other.top)
                val iR = minOf(best.right, other.right);
                val iB = minOf(best.bottom, other.bottom)
